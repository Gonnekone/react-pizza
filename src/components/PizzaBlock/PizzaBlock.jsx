...
function PizzaBlock({id, imageUrl, title, types, sizes, price}) {
    ...
    const cartItem = useSelector(state => state.cart.items.find(item => item.id === id
        && item.type === types[activeTypeIndex] && item.size === sizes[activeSizeIndex]));
                    ...
                    <ul>
                        {
                            types.map((type, currentIndex) => (
                                <li
                                    onClick={() => setActiveTypeIndex(currentIndex)}
                                    className={activeTypeIndex === currentIndex ? "active" : ""}
                                    key={type}
                                >
                                    {typeNames[type]}
                                </li>
                    ...
                    <ul>
                        {
                            sizes.map((size, currentIndex) => (
                                <li
                                    onClick={() => setActiveSizeIndex(currentIndex)}
                                    className={activeSizeIndex === currentIndex ? "active" : ""}
                                    key={size}
                                >
                                    {size} см.
                                </li>
